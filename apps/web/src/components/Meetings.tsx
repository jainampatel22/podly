...
export default function Meetings() {
  ...
  if (!session) {
    redirect(`/sign-in?callbackUrl=/explore/meetings`)
  }
        ...
        <div className="relative z-10">
          ...
          {invites.length === 0 ? (
            <div className="text-center px-4 mt-10">
              <div className="italic text-black text-3xl sm:text-5xl">
                You have 0 <span className="bg-gradient-to-r from-blue-600 via-purple-600 to-blue-600 bg-clip-text text-transparent">Invites</span>.
              </div>
              <Link href="/schedule">
                <div className="text-slate-500 italic text-xl sm:text-2xl mt-4">
                  Go and schedule<span className="underline ml-1">now!</span>
                </div>
              </Link>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-6 px-4 sm:px-6 lg:px-10 py-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {invites.map((invite) => (
                <Card key={invite.id} className="p-0 max-w-sm w-full mx-auto shadow-none border-none hover:scale-105 transition-transform duration-300">
                  <MagicCard
